			...
			if (v:IsValid() or v:IsPlayer()) and (v.forcefielded == false or v.forcefielded == nil) then
				...
				while i < v:GetPhysicsObjectCount() do
					...
					if phys:IsValid() then
						...
						local F_dir = (v:GetPos() - pos):GetNormal() * (self.DEFAULT_PHYSFORCE or 0)
					...
					if v:IsPlayer() then
						...
						local dist = (pos - v:GetPos()):Length()
						local relation = math.Clamp((self.CURRENTRANGE - dist) / self.CURRENTRANGE, 0, 1)
						local F_dir = (v:GetPos() - pos):GetNormal() * (self.DEFAULT_PHYSFORCE_PLYAIR or 0)
						v:SetVelocity(F_dir)
					end
					...
					if v:IsPlayer() and v:IsOnGround() then
						...
						local dist = (pos - v:GetPos()):Length()
						local relation = math.Clamp((self.CURRENTRANGE - dist) / self.CURRENTRANGE, 0, 1)
						local F_dir = (v:GetPos() - pos):GetNormal() * (self.DEFAULT_PHYSFORCE_PLYGROUND or 0)
						v:SetVelocity(F_dir)
					end
					if v:IsNPC() then
						v:TakeDamageInfo(dmg)
